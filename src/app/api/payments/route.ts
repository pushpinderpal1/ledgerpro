...
import { idempotencyShortCircuit, idempotencyRecord } from '@/lib/security/idempotency'
...
export async function POST(req: NextRequest) {
  ...
  // Honor Idempotency-Key header to make safe retries safe.
  const cached = await idempotencyShortCircuit(req, body.entityId, body)
  ...
  try {
    ...
    await idempotencyRecord(req, body.entityId, auth.session?.userId, body, payment, 201)
